...
Prove production/preview login with the dedicated smoke user, then at least one post-auth path (Today load). Clean-skips when smoke secrets are unset so the gate stays green.
...
- `post-auth-today` Today chrome loads after login (no 500).
